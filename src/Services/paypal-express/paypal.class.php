<?php
	...
	class MyPayPal extends connectionDB{
		...
		public function getPriceProduct( $getOneItem = FALSE )
		{
			...
			echo $sql = "SELECT precio, descripcion FROM ".$this->prefijo."pricing WHERE id = $planId AND estado = 1 ";
			foreach ($this->conn->query($sql) as $row) {
				$pay = $row['precio'];
				$aData = number_format($pay, 2, '.', '');
				$description = $row['descripcion'];
			}
			return array("pricing" => $aData, "description" => $description );
		...
		public function getLastId()
		{
			...
			$this->conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "SELECT id  FROM ".$this->prefijo."pagos WHERE ph_id_transaction != '' AND ph_active = 1 ORDER BY id DESC LIMIT 1";
			foreach ($this->conn->query($sql) as $row) {
				$id = $row['id'];
			}
			return $id;
		...
		public function createOrder($token){
			...
			if( count($items_cart) > 0)
			{
				...
				{  
					...
					$gsent = $this->conn->prepare( "INSERT INTO ".$this->prefijo."tmp_pagos ( 
										tmp_email ,tmp_picture, tmp_text,  tmp_link,  tmp_id_token, tmp_created_at, tmp_completed ) 
										VALUES ( ?,?,?,?,?, NOW(), ? )" 
									);
					$gsent->execute(
							array( $_SESSION['email'], $_SESSION['picture'],  $_SESSION['description'], $_SESSION['link'], $token, 0 ) 
						);
					/*
					$gsent->execute(
		...
		public function updateOrder( $httpParsedResponseAr )
		{
			...
			if( isset($httpParsedResponseAr) )
			{
				try
				{
					$transactionID = urldecode($httpParsedResponseAr["PAYMENTREQUESTINFO_0_TRANSACTIONID"]);
					$tokenID =  urldecode($httpParsedResponseAr["TOKEN"]);
					$payerID = urldecode($httpParsedResponseAr["PAYERID"]);
					$httpResponse = serialize($httpParsedResponseAr);
					$payedATM = urldecode($httpParsedResponseAr['PAYMENTREQUEST_0_AMT']);
					
					
					$sql= "SELECT * FROM ".$this->prefijo."tmp_pagos WHERE tmp_id_token = :token"; 
					$stmt = $this->conn->prepare($sql);
					$stmt->bindParam(':token', $tokenID, PDO::PARAM_INT); 
					$select = $stmt->execute();
					...
					if ( $select )
					{
						...
						if( count($res) > 0 )
						{
							...
							$gsent = $this->conn->prepare( "INSERT INTO ".$this->prefijo."pagos ( 
												cli_id, ph_email ,ph_picture, ph_text,  ph_link, ph_active, ph_payed, ph_id_payer,  ph_id_token, ph_id_transaction, ph_http_parsed_response, ph_created_at ) 
												VALUES ( ?, ?,?,?,?,?,?,?,?,?,?, NOW() )" 
											);
							$status = $gsent->execute(
									array( $_SESSION['clientId'], $email, $picture, $description, $link, 1, $payedATM, $payerID, $tokenID, $transactionID, $httpResponse ) 
								);
							if( $status )
							{
								$gsent = $this->conn->prepare( "DELETE FROM ".$this->prefijo."tmp_pagos  WHERE tmp_id_token = ?");
								$gsent->execute(array( $tokenID ) );
							}
						}
					}
					$status = 1;
