...
(function (factory) {
	...
	Tree.prototype = {
		...
		selectItem: function selectItem(el) {
			...
			if (this.options.multiSelect) {
				...
				$all.removeClass('tree-selected')
					.find('.fa').removeClass('fa-check').addClass('fa-circle');
				data.push(selData);
			}
			...
			if ($el.hasClass('tree-selected')) {
				...
				if ($icon.hasClass('fa-check') || $icon.hasClass('fa-circle')) {
		...
		openFolder: function openFolder(el, ignoreRedundantOpens) {
			...
			if (!ignoreRedundantOpens && $el.find('.fa-folder-open').length && !this.options.ignoreRedundantOpens) {
				this.closeFolder(el);
			}
